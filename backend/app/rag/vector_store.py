...
def get_gemini_client() -> Optional[genai.Client]:
    ...
    if api_key:
        try:
            ...
            print(f"[Embeddings] Error initializing Gemini client: {e}")
    return None
...
def _embed_chunk(idx: int, chunk: str, client: Optional[genai.Client]) -> Dict[str, Any]:
    ...
    if client:
        try:
            ...
            print(f"[Embeddings] Failed to generate embedding for chunk {idx}: {e}")
...
def process_document_background(document_id: str, agent_id: str, text: str) -> None:
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
            futures = [
                executor.submit(_embed_chunk, idx, chunk, client)
        ...
        print(f"[Embeddings] Background processing failed for document {document_id}: {e}")
...
def retrieve_context(agent_id: str, query: str, top_k: int = 3) -> List[Dict[str, Any]]:
    ...
    # Attempt embedding-based similarity if Gemini is available and embeddings exist in DB
    has_embeddings = all(c.get("embedding") for c in chunks)
    use_embedding_search = client and has_embeddings
    ...
    if use_embedding_search:
        try:
            ...
            print(f"[Embeddings] Embedding search failed, falling back to keyword matching: {e}")
            use_embedding_search = False

    # Fallback: local keyword (TF cosine) search
    if not use_embedding_search or not scored_chunks:
